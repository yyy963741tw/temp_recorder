...
//LCD�Ļ�����ɫ�ͱ���ɫ	   
u32 FRONT_COLOR=0xFF000000;		//������ɫ
u32 BACK_COLOR =0xFFFFFFFF;  	//����ɫ

_tftlcd_data tftlcd_data;//����LCD��Ҫ����
...
void LCD_Display_Dir(u8 dir)
{
	...
	if(lcdltdc.pwidth!=0)   //�����RGB��
	{
		...
		tftlcd_data.width=lcdltdc.width;
		tftlcd_data.height=lcdltdc.height;
		return;
	}
...
//	defined(TFTLCD_ILI9488)||defined(TFTLCD_ST7793)||defined(TFTLCD_ILI9325)|| \
//	defined(TFTLCD_ILI9327)||defined(TFTLCD_NT35510)
//
...
void TFTLCD_Init(void *init(void))
{
	u16 i=0;
	
#if	defined(TFTLCD_RGB4342)||defined(TFTLCD_RGB7084)||defined(TFTLCD_RGB7016)|| \
	defined(TFTLCD_RGB7018)||defined(TFTLCD_RGB8016)||defined(TFTLCD_RGB1018) 
//init();
	tftlcd_data.id=LTDC_PanelID_Read();	//����Ƿ���RGB������
...
//	LCD_WriteCmd(0xC2,0x00);	LCD_WriteData(0x00,0x03);
	...
	*/
	...
	LCD_Display_Dir(TFTLCD_DIR);		//Ĭ��Ϊ����
	LCD_Clear(WHITE);
//    LTDC_Clear(RED);			//����
...
{  
	...
	if(lcdltdc.pwidth!=0)	//�����RGB��
	{
		for(i=0;i<height;i++)
		{
			for(j=0;j<width;j++)
			{
				LCD_DrawFRONT_COLOR(sx+j,sy+i,color[i*width+j]);
			}
		}	
	}
	else
...
void LCD_DrawPoint(u16 x,u16 y)
{
	if(lcdltdc.pwidth!=0)//�����RGB��
	{
		LTDC_Draw_Point(x,y,FRONT_COLOR);
	}
	else
	{
...
//x,y:����
//color:��ɫ
void LCD_DrawFRONT_COLOR(u16 x,u16 y,u32 color)
{	   
	if(lcdltdc.pwidth!=0)//�����RGB��
	{
...
/**
 * @brief  直接繪製一個漢字的點陣資料 (除錯專用)
 * @param  x, y: 起始座標
 * @param  font_matrix: 指向一個漢字點陣資料陣列的指標 (例如 CnChar32x29[0].Msk)
 * @retval None
 */
void LCD_ShowHZ_Debug(u16 x, u16 y, const unsigned char *font_matrix)
{
    ...
    // 每個漢字的點陣資料大小為 116 bytes (32寬 * 29高 / 8 bits/byte)
    for (i = 0; i < 116; i++)
    {
        byte_data = font_matrix[i]; // 讀取一個位元組的點陣資料

        for (k = 0; k < 8; k++) // 處理這個位元組中的 8 個像素點
        {
            if ((byte_data << k) & 0x80) // 從最高位元開始檢查
            {
                // 如果該位元是 1，用前景色畫點
                LTDC_Draw_Point(x_pos, y_pos, FRONT_COLOR);
            }
            else
            {
                // 如果該位元是 0，用背景色畫點 (或不畫)
                LTDC_Draw_Point(x_pos, y_pos, BACK_COLOR);
            }

            x_pos++; // X 座標右移一點
            if (x_pos >= x + 32) // 如果畫完了一行 (32點寬)
            {
                x_pos = x;   // X 座標回到起點
                y_pos++;     // Y 座標換到下一行
            }
