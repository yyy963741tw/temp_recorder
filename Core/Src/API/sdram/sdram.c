...
SDRAM_CH SDRAM_CHS[SDRAM_TOTAL];
...
void SDRAM_Init(void *init(void), SDRAM_HandleTypeDef  *hsdram, unsigned int idx)
{
	init();
	SDRAM_CHS[idx].hsdram =				hsdram;
	SDRAM_CHS[idx].SDRAM_idx =			idx;
	SDRAM_Initialization_Sequence(idx);//����SDRAM��ʼ������
}

//����SDRAM��ʼ������
void SDRAM_Initialization_Sequence(unsigned int idx)
{
    u32 temp=0;
    //SDRAM��������ʼ������Ժ���Ҫ��������˳���ʼ��SDRAM
    SDRAM_Send_Cmd(idx, 0,FMC_SDRAM_CMD_CLK_ENABLE,1,0); //ʱ������ʹ��
    delay_us(500);                                  //������ʱ200us
    SDRAM_Send_Cmd(idx, 0,FMC_SDRAM_CMD_PALL,1,0);       //�����д洢��Ԥ���
    SDRAM_Send_Cmd(idx, 0,FMC_SDRAM_CMD_AUTOREFRESH_MODE,8,0);//������ˢ�´���
    ...
    SDRAM_Send_Cmd(idx, 0,FMC_SDRAM_CMD_LOAD_MODE,1,temp);   //����SDRAM��ģʽ�Ĵ���
    
    //ˢ��Ƶ�ʼ�����(��SDCLKƵ�ʼ���),���㷽��:
	  //COUNT=SDRAMˢ������/����-20=SDRAMˢ������(us)*SDCLKƵ��(Mhz)/����
    //����ʹ�õ�SDRAMˢ������Ϊ64ms,SDCLK=180/2=90Mhz,����Ϊ8192(2^13).
	  //����,COUNT=64*1000*90/8192-20=683
	HAL_SDRAM_ProgramRefreshRate(SDRAM_CHS[idx].hsdram,683);
...
//regval:ģʽ�Ĵ����Ķ���
//����ֵ:0,����;1,ʧ��.
u8 SDRAM_Send_Cmd(unsigned int idx, u8 bankx,u8 cmd,u8 refresh,u16 regval)
{
    u32 target_bank=0;
    ...
    if(HAL_SDRAM_SendCommand(SDRAM_CHS[idx].hsdram,&Command,0X1000)==HAL_OK) //��SDRAM��������
    {
        return 0;  
    }
    else return 1;    
